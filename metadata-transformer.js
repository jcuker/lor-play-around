...
async function downloadSets() {
  ...
  for (let i = 0; i < numberOfSets; i++) {
    ...
    try {
      ...
      fs.writeFileSync(setMetadataOutputPath, JSON.stringify(unzippedMetadata));
    } catch (err) {
...
async function main() {
  ...
  try {
    ...
    const regions = new Set(transformed.flatMap((c) => c.regions));
    ...
    transformed.forEach((c) => {
      ...
      delete c.regions;
      for (const region of regions) toWrite[region].push(c);
    });
